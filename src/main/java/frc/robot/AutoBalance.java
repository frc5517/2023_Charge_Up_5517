...
public class AutoBalance {
    ...
    public double getPitch() {
        return Math.atan2((-mRioAccel.getX()),
                (Math.sqrt(mRioAccel.getY() * mRioAccel.getY() + mRioAccel.getZ() * mRioAccel.getZ()) * 57.3) + 0); // Change 0 to make smartdashboard value as close to 0 as possible
    }

    public double getRoll() { 
        return (Math.atan2(mRioAccel.getY(), mRioAccel.getZ()) * 57.3) + 95; // Change 0 to make smartdashboard value as close to 0 as possible
    }
    ...
    public double getTilt() { // 
        ...
        double roll = getRoll();
        if ((pitch + roll) >= 0) {
            return (Math.sqrt(pitch * pitch + roll * roll)) + 0; // Change 0 to make smartdashboard value as close to 0 as possible
        } else {
            return -((Math.sqrt(pitch * pitch + roll * roll)) + 0); // Change 0 to make smartdashboard value as close to 0 as possible
        }
    }
    ...
    public double autoBalanceRoutine() {
        switch (state) {
            ...
            case 0:
                if (getRoll() > onChargeStationDegree) {
                    debounceCount++;
                }
            ...
            case 2:
                if (Math.abs(getRoll()) <= levelDegree / 2) {
                    debounceCount++;
                }
                if (debounceCount > secondsToTicks(debounceTime)) {
                    state = 4;
